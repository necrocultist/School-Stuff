...
normal_fastqs = [FastqReader("/workdir/fastq/SRR7890851_1.fastq.gz", name="normal", read=1),
                 FastqReader("/workdir/fastq/SRR7890851_2.fastq.gz", name="normal", read=2)]

tumor_fastqs = [FastqReader("/workdir/fastq/SRR7890850_1.fastq.gz", name="tumor", read=1),
                FastqReader("/workdir/fastq/SRR7890850_2.fastq.gz", name="tumor", read=2)]
...
# Creating mappers
mapper_normal = Mapper(input_step=trimmer_normal, library="bowtie", params={
                            "read_groups": {
                                "ID": "0",
                                "SM": "normal",
                                "PU": "0",
...
mapper_tumor = Mapper(input_step=trimmer_tumor, library="bowtie",params={
                            "read_groups": {
                                "ID": "0",
                                "SM": "tumor",
                                "PU": "0",
...
basecal_normal = Recalibrator(input_step=mdup_normal, bed_file="/workdir/ces_hg38.bed")
basecal_tumor = Recalibrator(input_step=mdup_tumor, bed_file="/workdir/ces_hg38.bed")

# Creating variant callers
somatic_sniper = VariantCaller(library="somaticsniper", germline=basecal_normal, tumor=basecal_tumor, bed_file="/workdir/ces_hg38.bed")
